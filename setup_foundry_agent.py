#!/usr/bin/env python3
"""
One-shot setup: create (or refresh) a Foundry agent that is wired to the
``chart-Automation`` Azure AI Search index, so you can chat with it in the
Foundry portal Agent Playground.

Pipeline:
    1) analyze_pptx_vision.py       -> deck.vision.slides.jsonl
    2) ingest_vision_to_search.py   -> chart-Automation index in Azure AI Search
    3) setup_foundry_agent.py       <-- this script: creates the Foundry agent

After this runs, open the Foundry portal -> Agents -> pick the agent (name
defaults to ``chart-automation-agent``) -> Playground, and start chatting.
The agent answers are grounded via the AI Search index attached as a tool.
...
Env (.env auto-loaded):
    ...
    SEARCH_CONNECTION_ID        Full ARM id of the Azure AI Search connection
                                in the Foundry project. Get it from:
